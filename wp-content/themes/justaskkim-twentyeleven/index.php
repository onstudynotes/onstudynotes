<?php /* Template Name: Home Page */ ?>
<!DOCTYPE html>
<html dir="ltr" lang="en-US">
  <head>
    <title>
      onstudynotes | just better.
    </title>
    <link rel="stylesheet" type="text/css" media="all" href="http://onstudynotes.com/wp-content/themes/justaskkim-twentyeleven/style.css"/>
    <link rel="shortcut icon" href="http://onstudynotes.ca/onstudynotes/Homepage/favicon.png" type="image/x-icon"/>
    <style type="text/css">
      #site-title,#site-description{
        position:absolute!important;
        clip:rect(1px 1px 1px 1px);
        clip:rect(1px,1px,1px,1px)}
      body{
        border-collapse:collapse;
        margin-top:0;
        margin-left:0;
        margin-right:0;
        border:0;
        border-top:0;
        border-width:0;
        border-spacing:0;
        border-style:none;
        margin:0;
        padding:0;
        text-align:center}
      table,tr,td,th{
        border-collapse:collapse;
        margin-top:0;
        margin-bottom 0;
        margin-left:auto;
        margin-right:auto;
        border:0;
        border-top:0;
        border-width:0;
        border-spacing:0;
        border-style:none;
        margin:0;
        padding:0;
        text-align:center}
      .panels{
        width:299px;
        height:590px;
        border-collapse:collapse;
        margin-top:0;
        margin-left:0;
        margin-right:0;
        border:0;
        border-top:0;
        border-width:0;
        border-spacing:0;
        border-style:none;
        margin:0;
        padding:0}
      .panelspadding{
        width:7px;
        height:590px}
      .sidepadding{
        width:5px;
        height:590px}
      .start3colour{
      	background-color:#c56db7
      	}
      	
    </style>
  </head>
  <body class="page page-id-959 page-template-default singular two-column right-sidebar">
    <div id="page" class="hfeed">
      <header id="branding" role="banner">
        <hgroup>
          <h1 id="site-title">
            <span>
              <a href="http://onstudynotes.com/" title="onstudynotes" rel="home">
              </a>
            </span>
          </h1>
          <h2 id="site-description">
          </h2>
        </hgroup>
        <a href="http://onstudynotes.com/">
          <img src="http://farm9.staticflickr.com/8494/8373690305_f3f4e5dabd_b.jpg" width="1000" height="90" alt=""/>
        </a>
        <div class="only-search with-image">
          <form method="get" id="searchform" action="http://onstudynotes.com/">
            <label for="s" class="assistive-text">
              Search
            </label>
            <input type="text" class="field" name="s" id="s" placeholder="&quot;SPH3U 11 Physics&quot;"/>
            <input type="submit" class="submit" name="submit" id="searchsubmit" value="Search"/>
          </form>
        </div>
        <nav id="access" role="navigation">
          <h3 class="assistive-text">
            Main menu
          </h3>
          <div class="menu-main-menu-container">
            <ul id="menu-main-menu" class="menu">
              <li id="menu-item-635" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-home menu-item-635">
                <a href="http://onstudynotes.com">
                  Home
                </a>
              </li>
              <li id="menu-item-637" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-637">
                <a href="http://onstudynotes.com/average-calculator/">
                  Average Calculator
                </a>
              </li>
              <li id="menu-item-868" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-868">
                <a href="http://onstudynotes.com/submit/">
                  Submit
                </a>
              </li>
              <li id="menu-item-1029" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-1029">
                <a href="http://onstudynotes.com/help/">
                  Help
                </a>
                <ul class="sub-menu">
                  <li id="menu-item-1030" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-1030">
                    <a href="http://onstudynotes.com/howtofind/">
                      How do I Find Notes ?
                    </a>
                  </li>
                  <li id="menu-item-1036" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-1036">
                    <a href="http://onstudynotes.com/howtosubmit/">
                      How do I submit notes ?
                    </a>
                  </li>
                </ul>
              </li>
              <li id="menu-item-638" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-638">
                <a href="http://onstudynotes.com/about/">
                  About
                </a>
                <ul class="sub-menu">
                  <li id="menu-item-1035" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-1035">
                    <a href="http://onstudynotes.com/about-us/">
                      About Us
                    </a>
                  </li>
                  <li id="menu-item-639" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-639">
                    <a href="http://onstudynotes.com/contact-us/">
                      Contact Us
                    </a>
                  </li>
                  <li id="menu-item-641" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-641">
                    <a>
                      Social
                    </a>
                    <ul class="sub-menu">
                      <li id="menu-item-642" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-642">
                        <a href="http://www.facebook.com/onstudynotes">
                          Facebook
                        </a>
                      </li>
                      <li id="menu-item-643" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-643">
                        <a href="http://www.twitter.com/onstudynotes">
                          Twitter
                        </a>
                      </li>
                      <li id="menu-item-644" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-644">
                        <a href="http://youtube.com/onstudynotes">
                          YouTube
                        </a>
                      </li>
                    </ul>
                  </li>
                </ul>
              </li>
            </ul>
          </div>
        </nav>
      </header>
      <table width=922px height=590px cellspacing=0px align="center">
        <tr width=922px cellspacing=0px>
          <td class="sidepadding" cellspacing=0px>
          </td>
          <td class="panels" cellspacing=0px>
            <iframe src="http://onstudynotes.com/wp-content/themes/justaskkim-twentyeleven/subjects.html" name="subjects" class="panels" scrolling="no">
            </iframe>
          </td>
        </td>
      <td class="panelspadding">
      </td>
      <td class="panels">
        <iframe src="http://onstudynotes.com/wp-content/themes/justaskkim-twentyeleven/start2.html" name="grades" class="panels" scrolling="no">
        </iframe>
      </td>
      <td class="panelspadding">
      </td>
      <td class="panels">
        <iframe src="http://onstudynotes.com/wp-content/themes/justaskkim-twentyeleven/start3.html" name="notes" class="panels" scrolling="no">
        </iframe>
      </td>
      <td class="sidepadding" cellspacing=0px>
      </td>
    </tr>
    
  </table>
  <center>
  <table width=912px height=100px cellspacing=0px align="center">
  	<tr width=912px class="start3colour" height=10px cellspacing=0px>
          <td height=5px cellspacing=0px>
          </td> </tr>
  
  
    <tr width=912px class="start3colour" height=100px cellspacing=0px>
          <td height=100px cellspacing=0px>
          
          <a href="https://itunes.apple.com/ca/app/onstudynotes/id789775061">
          <img src="https://farm4.staticflickr.com/3887/14376520224_d9f397a56b_o_d.png" height="100%" width="90%">
		  </a>
		


	</td>
	</tr>
  </table>
</center>
</footer>
</div>
</body>
</html>